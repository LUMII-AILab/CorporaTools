<?xml version="1.0" encoding="UTF-8"?>
<lvadata xmlns="http://ufal.mff.cuni.cz/pdt/pml/">
  <head>
    <schema href="lvaschema.xml" />
    <references>
      <reffile id="m" name="mdata" href="../c70-Cairo.m" />
      <reffile id="w" name="wdata" href="../c70-Cairo.w" />
    </references>
  </head>
  <meta>
    <annotation_info>
      <desc>CoNLL automātiskais konvertors, 0.3,  Wed Nov 22 13:02:10 2017</desc>
    </annotation_info>
  </meta>
  <trees>
    <LM id="a-c70-Cairo-p1s1">
      <s.rf>m#m-c70-Cairo-p1s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p1s1w2">
              <m.rf id="m-c70-Cairo-p1s1w2">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p1w2">
                  <token>uzrakstīja</token>
                </w.rf>
                <form>uzrakstīja</form>
                <lemma>uzrakstīt</lemma>
                <tag>vmnist330an</tag>
              </m.rf>
              <role>pred</role>
              <ord>2</ord>
              <children>
                <node id="a-c70-Cairo-p1s1w1">
                  <m.rf id="m-c70-Cairo-p1s1w1">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p1w1">
                      <token>Meitene</token>
                    </w.rf>
                    <form>Meitene</form>
                    <lemma>meitene</lemma>
                    <tag>ncfsn5</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>1</ord>
                </node>
                <node id="a-c70-Cairo-p1s1w5">
                  <m.rf id="m-c70-Cairo-p1s1w5">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p1w5">
                      <token>draudzenei</token>
                      <no_space_after>1</no_space_after>
                    </w.rf>
                    <form>draudzenei</form>
                    <lemma>draudzene</lemma>
                    <tag>ncfsd5</tag>
                  </m.rf>
                  <role>obj</role>
                  <ord>5</ord>
                  <children>
                    <node id="a-c70-Cairo-p1s1w4">
                      <m.rf id="m-c70-Cairo-p1s1w4">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p1w4">
                          <token>savai</token>
                        </w.rf>
                        <form>savai</form>
                        <lemma>sava</lemma>
                        <tag>ps0fsdn</tag>
                      </m.rf>
                      <role>attr</role>
                      <ord>4</ord>
                    </node>
                  </children>
                </node>
                <node id="a-c70-Cairo-p1s1w3">
                  <m.rf id="m-c70-Cairo-p1s1w3">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p1w3">
                      <token>vēstuli</token>
                    </w.rf>
                    <form>vēstuli</form>
                    <lemma>vēstule</lemma>
                    <tag>ncfsa5</tag>
                  </m.rf>
                  <role>obj</role>
                  <ord>3</ord>
                </node>
              </children>
            </node>
            <node id="a-c70-Cairo-p1s1w6">
              <m.rf id="m-c70-Cairo-p1s1w6">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p1w6">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>6</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p2s1">
      <s.rf>m#m-c70-Cairo-p2s1</s.rf>
      <children>
        <node id="a-c70-Cairo-p2s1w1">
          <m.rf id="m-c70-Cairo-p2s1w1">
            <src.rf>semi-automatic</src.rf>
            <w.rf id="w-c70-Cairo-p2w1">
              <token>Man</token>
            </w.rf>
            <form>Man</form>
            <lemma>es</lemma>
            <tag>pp10sdn</tag>
          </m.rf>
          <role>det</role>
          <ord>1</ord>
        </node>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p2s1w7">
              <m.rf id="m-c70-Cairo-p2s1w7">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p2w7">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>7</ord>
            </node>
            <node id="a-c70-Cairo-p2s1w2">
              <m.rf id="m-c70-Cairo-p2s1w2">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p2w2">
                  <token>šķiet</token>
                  <no_space_after>1</no_space_after>
                </w.rf>
                <form>šķiet</form>
                <lemma>šķist</lemma>
                <tag>vmnipi130an</tag>
              </m.rf>
              <role>pred</role>
              <ord>2</ord>
              <children>
                <node id="a-c70-Cairo-p2s1x1">
                  <role>subjCl</role>
                  <children>
                    <pmcinfo>
                      <pmctype>subrCl</pmctype>
                      <children>
                        <node id="a-c70-Cairo-p2s1w5">
                          <m.rf id="m-c70-Cairo-p2s1w5">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p2w5">
                              <token>līst</token>
                            </w.rf>
                            <form>līst</form>
                            <lemma>līt</lemma>
                            <tag>vmnipi130an</tag>
                          </m.rf>
                          <role>pred</role>
                          <ord>5</ord>
                          <children>
                            <node id="a-c70-Cairo-p2s1w6">
                              <m.rf id="m-c70-Cairo-p2s1w6">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p2w6">
                                  <token>lietus</token>
                                  <no_space_after>1</no_space_after>
                                </w.rf>
                                <form>lietus</form>
                                <lemma>lietus</lemma>
                                <tag>ncmsn3</tag>
                              </m.rf>
                              <role>subj</role>
                              <ord>6</ord>
                            </node>
                          </children>
                        </node>
                        <node id="a-c70-Cairo-p2s1w4">
                          <m.rf id="m-c70-Cairo-p2s1w4">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p2w4">
                              <token>ka</token>
                            </w.rf>
                            <form>ka</form>
                            <lemma>ka</lemma>
                            <tag>cs</tag>
                          </m.rf>
                          <role>conj</role>
                          <ord>4</ord>
                        </node>
                        <node id="a-c70-Cairo-p2s1w3">
                          <m.rf id="m-c70-Cairo-p2s1w3">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p2w3">
                              <token>,</token>
                            </w.rf>
                            <form>,</form>
                            <lemma>,</lemma>
                            <tag>zc</tag>
                          </m.rf>
                          <role>punct</role>
                          <ord>3</ord>
                        </node>
                      </children>
                    </pmcinfo>
                  </children>
                </node>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p3s1">
      <s.rf>m#m-c70-Cairo-p3s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p3s1x1">
              <role>pred</role>
              <children>
                <node id="a-c70-Cairo-p3s1w1">
                  <m.rf id="m-c70-Cairo-p3s1w1">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p3w1">
                      <token>Viņš</token>
                    </w.rf>
                    <form>Viņš</form>
                    <lemma>viņš</lemma>
                    <tag>pp3msnn</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>1</ord>
                </node>
                <xinfo>
                  <xtype>xPred</xtype>
                  <tag>vm0is0030an[modals00]</tag>
                  <children>
                    <node id="a-c70-Cairo-p3s1w2">
                      <m.rf id="m-c70-Cairo-p3s1w2">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p3w2">
                          <token>centās</token>
                        </w.rf>
                        <form>centās</form>
                        <lemma>censties</lemma>
                        <tag>voyisi130an</tag>
                      </m.rf>
                      <role>mod</role>
                      <ord>2</ord>
                    </node>
                    <node id="a-c70-Cairo-p3s1w3">
                      <m.rf id="m-c70-Cairo-p3s1w3">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p3w3">
                          <token>atmest</token>
                        </w.rf>
                        <form>atmest</form>
                        <lemma>atmest</lemma>
                        <tag>vmnn0t1000n</tag>
                      </m.rf>
                      <role>basElem</role>
                      <ord>3</ord>
                      <children>
                        <node id="a-c70-Cairo-p3s1x2">
                          <role>obj</role>
                          <children>
                            <coordinfo>
                              <coordtype>crdParts</coordtype>
                              <children>
                                <node id="a-c70-Cairo-p3s1w5">
                                  <m.rf id="m-c70-Cairo-p3s1w5">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p3w5">
                                      <token>un</token>
                                    </w.rf>
                                    <form>un</form>
                                    <lemma>un</lemma>
                                    <tag>cc</tag>
                                  </m.rf>
                                  <role>conj</role>
                                  <ord>5</ord>
                                </node>
                                <node id="a-c70-Cairo-p3s1w6">
                                  <m.rf id="m-c70-Cairo-p3s1w6">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p3w6">
                                      <token>dzeršanu</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>dzeršanu</form>
                                    <lemma>dzeršana</lemma>
                                    <tag>ncfsa4</tag>
                                  </m.rf>
                                  <role>crdPart</role>
                                  <ord>6</ord>
                                </node>
                                <node id="a-c70-Cairo-p3s1w4">
                                  <m.rf id="m-c70-Cairo-p3s1w4">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p3w4">
                                      <token>smēķēšanu</token>
                                    </w.rf>
                                    <form>smēķēšanu</form>
                                    <lemma>smēķēšana</lemma>
                                    <tag>ncfsa4</tag>
                                  </m.rf>
                                  <role>crdPart</role>
                                  <ord>4</ord>
                                </node>
                              </children>
                            </coordinfo>
                          </children>
                        </node>
                      </children>
                    </node>
                  </children>
                </xinfo>
              </children>
            </node>
            <node id="a-c70-Cairo-p3s1w7">
              <m.rf id="m-c70-Cairo-p3s1w7">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p3w7">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>7</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p4s1">
      <s.rf>m#m-c70-Cairo-p4s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p4s1x1">
              <role>pred</role>
              <children>
                <node id="a-c70-Cairo-p4s1w2">
                  <m.rf id="m-c70-Cairo-p4s1w2">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p4w2">
                      <token>tu</token>
                    </w.rf>
                    <form>tu</form>
                    <lemma>tu</lemma>
                    <tag>pp20snn</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>2</ord>
                </node>
                <xinfo>
                  <xtype>xPred</xtype>
                  <tag>vm0ip002san[modals00]</tag>
                  <children>
                    <node id="a-c70-Cairo-p4s1w3">
                      <m.rf id="m-c70-Cairo-p4s1w3">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p4w3">
                          <token>gribi</token>
                        </w.rf>
                        <form>gribi</form>
                        <lemma>gribēt</lemma>
                        <tag>vonipi32san</tag>
                      </m.rf>
                      <role>mod</role>
                      <ord>3</ord>
                    </node>
                    <node id="a-c70-Cairo-p4s1w4">
                      <m.rf id="m-c70-Cairo-p4s1w4">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p4w4">
                          <token>iet</token>
                          <no_space_after>1</no_space_after>
                        </w.rf>
                        <form>iet</form>
                        <lemma>iet</lemma>
                        <tag>vmnn0ii000n</tag>
                      </m.rf>
                      <role>basElem</role>
                      <ord>4</ord>
                    </node>
                  </children>
                </xinfo>
              </children>
            </node>
            <node id="a-c70-Cairo-p4s1w1">
              <m.rf id="m-c70-Cairo-p4s1w1">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p4w1">
                  <token>Vai</token>
                </w.rf>
                <form>Vai</form>
                <lemma>vai</lemma>
                <tag>q</tag>
              </m.rf>
              <role>no</role>
              <ord>1</ord>
            </node>
            <node id="a-c70-Cairo-p4s1w5">
              <m.rf id="m-c70-Cairo-p4s1w5">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p4w5">
                  <token>?</token>
                </w.rf>
                <form>?</form>
                <lemma>?</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>5</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p5s1">
      <s.rf>m#m-c70-Cairo-p5s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p5s1x1">
              <role>no</role>
              <children>
                <pmcinfo>
                  <pmctype>address</pmctype>
                  <children>
                    <node id="a-c70-Cairo-p5s1w1">
                      <m.rf id="m-c70-Cairo-p5s1w1">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p5w1">
                          <token>Sem</token>
                          <no_space_after>1</no_space_after>
                        </w.rf>
                        <form>Sem</form>
                        <lemma>Sems</lemma>
                        <tag>npmsv1</tag>
                      </m.rf>
                      <role>basElem</role>
                      <ord>1</ord>
                    </node>
                    <node id="a-c70-Cairo-p5s1w2">
                      <m.rf id="m-c70-Cairo-p5s1w2">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p5w2">
                          <token>,</token>
                        </w.rf>
                        <form>,</form>
                        <lemma>,</lemma>
                        <tag>zc</tag>
                      </m.rf>
                      <role>punct</role>
                      <ord>2</ord>
                    </node>
                  </children>
                </pmcinfo>
              </children>
            </node>
            <node id="a-c70-Cairo-p5s1w5">
              <m.rf id="m-c70-Cairo-p5s1w5">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p5w5">
                  <token>!</token>
                </w.rf>
                <form>!</form>
                <lemma>!</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>5</ord>
            </node>
            <node id="a-c70-Cairo-p5s1w3">
              <m.rf id="m-c70-Cairo-p5s1w3">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p5w3">
                  <token>atver</token>
                </w.rf>
                <form>atver</form>
                <lemma>atvērt</lemma>
                <tag>vmnm0t12san</tag>
              </m.rf>
              <role>pred</role>
              <ord>3</ord>
              <children>
                <node id="a-c70-Cairo-p5s1w4">
                  <m.rf id="m-c70-Cairo-p5s1w4">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p5w4">
                      <token>logu</token>
                      <no_space_after>1</no_space_after>
                    </w.rf>
                    <form>logu</form>
                    <lemma>logs</lemma>
                    <tag>ncmsa1</tag>
                  </m.rf>
                  <role>obj</role>
                  <ord>4</ord>
                </node>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p6s1">
      <s.rf>m#m-c70-Cairo-p6s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p6s1w7">
              <m.rf id="m-c70-Cairo-p6s1w7">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p6w7">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>7</ord>
            </node>
            <node id="a-c70-Cairo-p6s1w2">
              <m.rf id="m-c70-Cairo-p6s1w2">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p6w2">
                  <token>lika</token>
                </w.rf>
                <form>lika</form>
                <lemma>likt</lemma>
                <tag>vmnist130an</tag>
              </m.rf>
              <role>pred</role>
              <ord>2</ord>
              <children>
                <node id="a-c70-Cairo-p6s1w4">
                  <m.rf id="m-c70-Cairo-p6s1w4">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p6w4">
                      <token>vīram</token>
                    </w.rf>
                    <form>vīram</form>
                    <lemma>vīrs</lemma>
                    <tag>ncmsd1</tag>
                  </m.rf>
                  <role>obj</role>
                  <ord>4</ord>
                  <children>
                    <node id="a-c70-Cairo-p6s1w3">
                      <m.rf id="m-c70-Cairo-p6s1w3">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p6w3">
                          <token>savam</token>
                        </w.rf>
                        <form>savam</form>
                        <lemma>savs</lemma>
                        <tag>ps0msdn</tag>
                      </m.rf>
                      <role>attr</role>
                      <ord>3</ord>
                    </node>
                  </children>
                </node>
                <node id="a-c70-Cairo-p6s1w5">
                  <m.rf id="m-c70-Cairo-p6s1w5">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p6w5">
                      <token>nomazgāt</token>
                    </w.rf>
                    <form>nomazgāt</form>
                    <lemma>nomazgāt</lemma>
                    <tag>vmnn0t2000n</tag>
                  </m.rf>
                  <role>spc</role>
                  <ord>5</ord>
                  <children>
                    <node id="a-c70-Cairo-p6s1w6">
                      <m.rf id="m-c70-Cairo-p6s1w6">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p6w6">
                          <token>mašīnu</token>
                          <no_space_after>1</no_space_after>
                        </w.rf>
                        <form>mašīnu</form>
                        <lemma>mašīna</lemma>
                        <tag>ncfsa4</tag>
                      </m.rf>
                      <role>obj</role>
                      <ord>6</ord>
                    </node>
                  </children>
                </node>
                <node id="a-c70-Cairo-p6s1w1">
                  <m.rf id="m-c70-Cairo-p6s1w1">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p6w1">
                      <token>Viņa</token>
                    </w.rf>
                    <form>Viņa</form>
                    <lemma>viņa</lemma>
                    <tag>pp3fsnn</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>1</ord>
                </node>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p7s1">
      <s.rf>m#m-c70-Cairo-p7s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p7s1w3">
              <m.rf id="m-c70-Cairo-p7s1w3">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p7w3">
                  <token>nokrāsoja</token>
                </w.rf>
                <form>nokrāsoja</form>
                <lemma>nokrāsot</lemma>
                <tag>vmnist230an</tag>
              </m.rf>
              <role>pred</role>
              <ord>3</ord>
              <children>
                <node id="a-c70-Cairo-p7s1w6">
                  <m.rf id="m-c70-Cairo-p7s1w6">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p7w6">
                      <token>krāsā</token>
                      <no_space_after>1</no_space_after>
                    </w.rf>
                    <form>krāsā</form>
                    <lemma>krāsa</lemma>
                    <tag>ncfsl4</tag>
                  </m.rf>
                  <role>spc</role>
                  <ord>6</ord>
                  <children>
                    <node id="a-c70-Cairo-p7s1w5">
                      <m.rf id="m-c70-Cairo-p7s1w5">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p7w5">
                          <token>sarkanā</token>
                        </w.rf>
                        <form>sarkanā</form>
                        <lemma>sarkans</lemma>
                        <tag>affslnp</tag>
                      </m.rf>
                      <role>attr</role>
                      <ord>5</ord>
                    </node>
                  </children>
                </node>
                <node id="a-c70-Cairo-p7s1w4">
                  <m.rf id="m-c70-Cairo-p7s1w4">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p7w4">
                      <token>žogu</token>
                    </w.rf>
                    <form>žogu</form>
                    <lemma>žogs</lemma>
                    <tag>ncmsa1</tag>
                  </m.rf>
                  <role>obj</role>
                  <ord>4</ord>
                </node>
                <node id="a-c70-Cairo-p7s1w2">
                  <m.rf id="m-c70-Cairo-p7s1w2">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p7w2">
                      <token>kaimiņi</token>
                    </w.rf>
                    <form>kaimiņi</form>
                    <lemma>kaimiņš</lemma>
                    <tag>ncmpn1</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>2</ord>
                  <children>
                    <node id="a-c70-Cairo-p7s1w1">
                      <m.rf id="m-c70-Cairo-p7s1w1">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p7w1">
                          <token>Pētera</token>
                        </w.rf>
                        <form>Pētera</form>
                        <lemma>Pēteris</lemma>
                        <tag>npmsg2</tag>
                      </m.rf>
                      <role>attr</role>
                      <ord>1</ord>
                    </node>
                  </children>
                </node>
              </children>
            </node>
            <node id="a-c70-Cairo-p7s1w7">
              <m.rf id="m-c70-Cairo-p7s1w7">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p7w7">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>7</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p8s1">
      <s.rf>m#m-c70-Cairo-p8s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p8s1w7">
              <m.rf id="m-c70-Cairo-p8s1w7">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p8w7">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>7</ord>
            </node>
            <node id="a-c70-Cairo-p8s1x1">
              <role>pred</role>
              <children>
                <node id="a-c70-Cairo-p8s1w2">
                  <m.rf id="m-c70-Cairo-p8s1w2">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p8w2">
                      <token>tētis</token>
                    </w.rf>
                    <form>tētis</form>
                    <lemma>tētis</lemma>
                    <tag>ncmsn2</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>2</ord>
                  <children>
                    <node id="a-c70-Cairo-p8s1w1">
                      <m.rf id="m-c70-Cairo-p8s1w1">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p8w1">
                          <token>Mans</token>
                        </w.rf>
                        <form>Mans</form>
                        <lemma>mans</lemma>
                        <tag>ps0msnn</tag>
                      </m.rf>
                      <role>attr</role>
                      <ord>1</ord>
                    </node>
                  </children>
                </node>
                <xinfo>
                  <xtype>xPred</xtype>
                  <tag>vm0ip003san[adjsmn]</tag>
                  <children>
                    <node id="a-c70-Cairo-p8s1w4">
                      <m.rf id="m-c70-Cairo-p8s1w4">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p8w4">
                          <token>foršāks</token>
                        </w.rf>
                        <form>foršāks</form>
                        <lemma>foršs</lemma>
                        <tag>afmsnnc</tag>
                      </m.rf>
                      <role>basElem</role>
                      <ord>4</ord>
                      <children>
                        <node id="a-c70-Cairo-p8s1x2">
                          <role>spc</role>
                          <children>
                            <xinfo>
                              <xtype>xSimile</xtype>
                              <tag>ps0msnn[compn]</tag>
                              <children>
                                <node id="a-c70-Cairo-p8s1w5">
                                  <m.rf id="m-c70-Cairo-p8s1w5">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p8w5">
                                      <token>nekā</token>
                                    </w.rf>
                                    <form>nekā</form>
                                    <lemma>nekā</lemma>
                                    <tag>cs</tag>
                                  </m.rf>
                                  <role>conj</role>
                                  <ord>5</ord>
                                </node>
                                <node id="a-c70-Cairo-p8s1w6">
                                  <m.rf id="m-c70-Cairo-p8s1w6">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p8w6">
                                      <token>tavējais</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>tavējais</form>
                                    <lemma>tavējs</lemma>
                                    <tag>armsnyp</tag>
                                  </m.rf>
                                  <role>basElem</role>
                                  <ord>6</ord>
                                </node>
                              </children>
                            </xinfo>
                          </children>
                        </node>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p8s1w3">
                      <m.rf id="m-c70-Cairo-p8s1w3">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p8w3">
                          <token>ir</token>
                        </w.rf>
                        <form>ir</form>
                        <lemma>būt</lemma>
                        <tag>vcnipii30an</tag>
                      </m.rf>
                      <role>auxVerb</role>
                      <ord>3</ord>
                    </node>
                  </children>
                </xinfo>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p9s1">
      <s.rf>m#m-c70-Cairo-p9s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p9s1x1">
              <role>basElem</role>
              <children>
                <coordinfo>
                  <coordtype>crdClauses</coordtype>
                  <children>
                    <node id="a-c70-Cairo-p9s1x2">
                      <role>crdPart</role>
                      <children>
                        <pmcinfo>
                          <pmctype>mainCl</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p9s1w2">
                              <m.rf id="m-c70-Cairo-p9s1w2">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p9w2">
                                  <token>ieguva</token>
                                </w.rf>
                                <form>ieguva</form>
                                <lemma>iegūt</lemma>
                                <tag>vmnist130an</tag>
                              </m.rf>
                              <role>pred</role>
                              <ord>2</ord>
                              <children>
                                <node id="a-c70-Cairo-p9s1w1">
                                  <m.rf id="m-c70-Cairo-p9s1w1">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p9w1">
                                      <token>Marija</token>
                                    </w.rf>
                                    <form>Marija</form>
                                    <lemma>marija</lemma>
                                    <tag>ncfsn4</tag>
                                  </m.rf>
                                  <role>subj</role>
                                  <ord>1</ord>
                                </node>
                                <node id="a-c70-Cairo-p9s1w3">
                                  <m.rf id="m-c70-Cairo-p9s1w3">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p9w3">
                                      <token>bronzu</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>bronzu</form>
                                    <lemma>bronza</lemma>
                                    <tag>ncfsa4</tag>
                                  </m.rf>
                                  <role>obj</role>
                                  <ord>3</ord>
                                </node>
                              </children>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p9s1x3">
                      <role>crdPart</role>
                      <children>
                        <pmcinfo>
                          <pmctype>mainCl</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p9s1w11">
                              <m.rf id="m-c70-Cairo-p9s1w11">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p9w11">
                                  <token>—</token>
                                </w.rf>
                                <form>—</form>
                                <lemma>–</lemma>
                                <tag>zd</tag>
                              </m.rf>
                              <reduction>vmnist130an(ieguva)</reduction>
                              <role>pred</role>
                              <ord>11</ord>
                              <children>
                                <node id="a-c70-Cairo-p9s1w10">
                                  <m.rf id="m-c70-Cairo-p9s1w10">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p9w10">
                                      <token>Džeina</token>
                                    </w.rf>
                                    <form>Džeina</form>
                                    <lemma>Džeina</lemma>
                                    <tag>npfsn4</tag>
                                  </m.rf>
                                  <role>subj</role>
                                  <ord>10</ord>
                                </node>
                                <node id="a-c70-Cairo-p9s1w12">
                                  <m.rf id="m-c70-Cairo-p9s1w12">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p9w12">
                                      <token>zeltu</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>zeltu</form>
                                    <lemma>zelts</lemma>
                                    <tag>ncmva1</tag>
                                  </m.rf>
                                  <role>obj</role>
                                  <ord>12</ord>
                                </node>
                              </children>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p9s1x4">
                      <role>crdPart</role>
                      <children>
                        <pmcinfo>
                          <pmctype>mainCl</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p9s1w6">
                              <m.rf id="m-c70-Cairo-p9s1w6">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p9w6">
                                  <token>—</token>
                                </w.rf>
                                <form>—</form>
                                <lemma>–</lemma>
                                <tag>zd</tag>
                              </m.rf>
                              <reduction>vmnist130an(ieguva)</reduction>
                              <role>pred</role>
                              <ord>6</ord>
                              <children>
                                <node id="a-c70-Cairo-p9s1w5">
                                  <m.rf id="m-c70-Cairo-p9s1w5">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p9w5">
                                      <token>Pēteris</token>
                                    </w.rf>
                                    <form>Pēteris</form>
                                    <lemma>Pēteris</lemma>
                                    <tag>npmsn2</tag>
                                  </m.rf>
                                  <role>subj</role>
                                  <ord>5</ord>
                                </node>
                                <node id="a-c70-Cairo-p9s1w7">
                                  <m.rf id="m-c70-Cairo-p9s1w7">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p9w7">
                                      <token>sudrabu</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>sudrabu</form>
                                    <lemma>sudrabs</lemma>
                                    <tag>ncmsa1</tag>
                                  </m.rf>
                                  <role>obj</role>
                                  <ord>7</ord>
                                </node>
                              </children>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p9s1w4">
                      <m.rf id="m-c70-Cairo-p9s1w4">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p9w4">
                          <token>,</token>
                        </w.rf>
                        <form>,</form>
                        <lemma>,</lemma>
                        <tag>zc</tag>
                      </m.rf>
                      <role>punct</role>
                      <ord>4</ord>
                    </node>
                    <node id="a-c70-Cairo-p9s1w8">
                      <m.rf id="m-c70-Cairo-p9s1w8">
                        <src.rf>manual</src.rf>
                        <w.rf id="w-c70-Cairo-p9w8">
                          <token>,</token>
                        </w.rf>
                        <form>,</form>
                        <lemma>,</lemma>
                        <tag>zc</tag>
                      </m.rf>
                      <role>punct</role>
                      <ord>8</ord>
                    </node>
                    <node id="a-c70-Cairo-p9s1w9">
                      <m.rf id="m-c70-Cairo-p9s1w9">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p9w9">
                          <token>un</token>
                        </w.rf>
                        <form>un</form>
                        <lemma>un</lemma>
                        <tag>cc</tag>
                      </m.rf>
                      <role>conj</role>
                      <ord>9</ord>
                    </node>
                  </children>
                </coordinfo>
              </children>
            </node>
            <node id="a-c70-Cairo-p9s1w13">
              <m.rf id="m-c70-Cairo-p9s1w13">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p9w13">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>13</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p10s1">
      <s.rf>m#m-c70-Cairo-p10s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p10s1w1">
              <m.rf id="m-c70-Cairo-p10s1w1">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p10w1">
                  <token>Vai</token>
                </w.rf>
                <form>Vai</form>
                <lemma>vai</lemma>
                <tag>q</tag>
              </m.rf>
              <role>no</role>
              <ord>1</ord>
            </node>
            <node id="a-c70-Cairo-p10s1w8">
              <m.rf id="m-c70-Cairo-p10s1w8">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p10w8">
                  <token>?</token>
                </w.rf>
                <form>?</form>
                <lemma>?</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>8</ord>
            </node>
            <node id="a-c70-Cairo-p10s1x1">
              <role>pred</role>
              <children>
                <node id="a-c70-Cairo-p10s1w2">
                  <m.rf id="m-c70-Cairo-p10s1w2">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p10w2">
                      <token>Igvasu</token>
                    </w.rf>
                    <form>Igvasu</form>
                    <lemma>Igvasu</lemma>
                    <tag>np0000</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>2</ord>
                </node>
                <xinfo>
                  <xtype>xPred</xtype>
                  <tag>vm0ip0030an[substsfn]</tag>
                  <children>
                    <node id="a-c70-Cairo-p10s1w3">
                      <m.rf id="m-c70-Cairo-p10s1w3">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p10w3">
                          <token>ir</token>
                        </w.rf>
                        <form>ir</form>
                        <lemma>būt</lemma>
                        <tag>vcnipii30an</tag>
                      </m.rf>
                      <role>auxVerb</role>
                      <ord>3</ord>
                    </node>
                    <node id="a-c70-Cairo-p10s1w7">
                      <m.rf id="m-c70-Cairo-p10s1w7">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p10w7">
                          <token>valsts</token>
                          <no_space_after>1</no_space_after>
                        </w.rf>
                        <form>valsts</form>
                        <lemma>valsts</lemma>
                        <tag>ncfsn6</tag>
                      </m.rf>
                      <role>basElem</role>
                      <ord>7</ord>
                      <children>
                        <node id="a-c70-Cairo-p10s1x2">
                          <role>attr</role>
                          <children>
                            <coordinfo>
                              <coordtype>crdParts</coordtype>
                              <children>
                                <node id="a-c70-Cairo-p10s1w4">
                                  <m.rf id="m-c70-Cairo-p10s1w4">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p10w4">
                                      <token>liela</token>
                                    </w.rf>
                                    <form>liela</form>
                                    <lemma>liels</lemma>
                                    <tag>affsnnp</tag>
                                  </m.rf>
                                  <role>crdPart</role>
                                  <ord>4</ord>
                                </node>
                                <node id="a-c70-Cairo-p10s1w5">
                                  <m.rf id="m-c70-Cairo-p10s1w5">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p10w5">
                                      <token>vai</token>
                                    </w.rf>
                                    <form>vai</form>
                                    <lemma>vai</lemma>
                                    <tag>cc</tag>
                                  </m.rf>
                                  <role>conj</role>
                                  <ord>5</ord>
                                </node>
                                <node id="a-c70-Cairo-p10s1w6">
                                  <m.rf id="m-c70-Cairo-p10s1w6">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p10w6">
                                      <token>maza</token>
                                    </w.rf>
                                    <form>maza</form>
                                    <lemma>mazs</lemma>
                                    <tag>affsnnp</tag>
                                  </m.rf>
                                  <role>crdPart</role>
                                  <ord>6</ord>
                                </node>
                              </children>
                            </coordinfo>
                          </children>
                        </node>
                      </children>
                    </node>
                  </children>
                </xinfo>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p11s1">
      <s.rf>m#m-c70-Cairo-p11s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p11s1w10">
              <m.rf id="m-c70-Cairo-p11s1w10">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p11w10">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>10</ord>
            </node>
            <node id="a-c70-Cairo-p11s1x1">
              <role>pred</role>
              <children>
                <xinfo>
                  <xtype>xPred</xtype>
                  <tag>vm0is0030ay[modals00]</tag>
                  <children>
                    <node id="a-c70-Cairo-p11s1w2">
                      <m.rf id="m-c70-Cairo-p11s1w2">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p11w2">
                          <token>izvēlēties</token>
                        </w.rf>
                        <form>izvēlēties</form>
                        <lemma>izvēlēties</lemma>
                        <tag>vmyn0t3000n</tag>
                      </m.rf>
                      <role>basElem</role>
                      <ord>2</ord>
                      <children>
                        <node id="a-c70-Cairo-p11s1x2">
                          <role>obj</role>
                          <children>
                            <coordinfo>
                              <coordtype>crdParts</coordtype>
                              <children>
                                <node id="a-c70-Cairo-p11s1w3">
                                  <m.rf id="m-c70-Cairo-p11s1w3">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p11w3">
                                      <token>ne</token>
                                    </w.rf>
                                    <form>ne</form>
                                    <lemma>ne</lemma>
                                    <tag>cc</tag>
                                  </m.rf>
                                  <role>conj</role>
                                  <ord>3</ord>
                                </node>
                                <node id="a-c70-Cairo-p11s1x3">
                                  <role>crdPart</role>
                                  <children>
                                    <xinfo>
                                      <xtype>namedEnt</xtype>
                                      <tag>npmsa0</tag>
                                      <children>
                                        <node id="a-c70-Cairo-p11s1w5">
                                          <m.rf id="m-c70-Cairo-p11s1w5">
                                            <src.rf>semi-automatic</src.rf>
                                            <w.rf id="w-c70-Cairo-p11w5">
                                              <token>Smitu</token>
                                              <no_space_after>1</no_space_after>
                                            </w.rf>
                                            <form>Smitu</form>
                                            <lemma>Smits</lemma>
                                            <tag>npmsa1</tag>
                                          </m.rf>
                                          <role>basElem</role>
                                          <ord>5</ord>
                                        </node>
                                        <node id="a-c70-Cairo-p11s1w4">
                                          <m.rf id="m-c70-Cairo-p11s1w4">
                                            <src.rf>semi-automatic</src.rf>
                                            <w.rf id="w-c70-Cairo-p11w4">
                                              <token>Pēteri</token>
                                            </w.rf>
                                            <form>Pēteri</form>
                                            <lemma>Pēteris</lemma>
                                            <tag>npmsa2</tag>
                                          </m.rf>
                                          <role>basElem</role>
                                          <ord>4</ord>
                                        </node>
                                      </children>
                                    </xinfo>
                                  </children>
                                </node>
                                <node id="a-c70-Cairo-p11s1x4">
                                  <role>crdPart</role>
                                  <children>
                                    <xinfo>
                                      <xtype>namedEnt</xtype>
                                      <tag>npfsa0</tag>
                                      <children>
                                        <node id="a-c70-Cairo-p11s1w8">
                                          <m.rf id="m-c70-Cairo-p11s1w8">
                                            <src.rf>semi-automatic</src.rf>
                                            <w.rf id="w-c70-Cairo-p11w8">
                                              <token>Mariju</token>
                                            </w.rf>
                                            <form>Mariju</form>
                                            <lemma>Marija</lemma>
                                            <tag>npfsa4</tag>
                                          </m.rf>
                                          <role>basElem</role>
                                          <ord>8</ord>
                                        </node>
                                        <node id="a-c70-Cairo-p11s1w9">
                                          <m.rf id="m-c70-Cairo-p11s1w9">
                                            <src.rf>semi-automatic</src.rf>
                                            <w.rf id="w-c70-Cairo-p11w9">
                                              <token>Braunu</token>
                                              <no_space_after>1</no_space_after>
                                            </w.rf>
                                            <form>Braunu</form>
                                            <lemma>Brauna</lemma>
                                            <tag>npfsa4</tag>
                                          </m.rf>
                                          <role>basElem</role>
                                          <ord>9</ord>
                                        </node>
                                      </children>
                                    </xinfo>
                                  </children>
                                </node>
                                <node id="a-c70-Cairo-p11s1w7">
                                  <m.rf id="m-c70-Cairo-p11s1w7">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p11w7">
                                      <token>ne</token>
                                    </w.rf>
                                    <form>ne</form>
                                    <lemma>ne</lemma>
                                    <tag>cc</tag>
                                  </m.rf>
                                  <role>conj</role>
                                  <ord>7</ord>
                                </node>
                                <node id="a-c70-Cairo-p11s1w6">
                                  <m.rf id="m-c70-Cairo-p11s1w6">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p11w6">
                                      <token>,</token>
                                    </w.rf>
                                    <form>,</form>
                                    <lemma>,</lemma>
                                    <tag>zc</tag>
                                  </m.rf>
                                  <role>punct</role>
                                  <ord>6</ord>
                                </node>
                              </children>
                            </coordinfo>
                          </children>
                        </node>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p11s1w1">
                      <m.rf id="m-c70-Cairo-p11s1w1">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p11w1">
                          <token>Nevarēja</token>
                        </w.rf>
                        <form>Nevarēja</form>
                        <lemma>varēt</lemma>
                        <tag>vonisi330ay</tag>
                      </m.rf>
                      <role>mod</role>
                      <ord>1</ord>
                    </node>
                  </children>
                </xinfo>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p12s1">
      <s.rf>m#m-c70-Cairo-p12s1</s.rf>
      <children>
        <node id="a-c70-Cairo-p12s1w1">
          <m.rf id="m-c70-Cairo-p12s1w1">
            <src.rf>semi-automatic</src.rf>
            <w.rf id="w-c70-Cairo-p12w1">
              <token>Viņiem</token>
            </w.rf>
            <form>Viņiem</form>
            <lemma>viņš</lemma>
            <tag>pp3mpdn</tag>
          </m.rf>
          <role>det</role>
          <ord>1</ord>
        </node>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p12s1w9">
              <m.rf id="m-c70-Cairo-p12s1w9">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p12w9">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>9</ord>
            </node>
            <node id="a-c70-Cairo-p12s1w2">
              <m.rf id="m-c70-Cairo-p12s1w2">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p12w2">
                  <token>nav</token>
                </w.rf>
                <form>nav</form>
                <lemma>būt</lemma>
                <tag>vmnipii30ay</tag>
              </m.rf>
              <role>pred</role>
              <ord>2</ord>
              <children>
                <node id="a-c70-Cairo-p12s1x1">
                  <role>subj</role>
                  <children>
                    <node id="a-c70-Cairo-p12s1x2">
                      <role>attrCl</role>
                      <children>
                        <pmcinfo>
                          <pmctype>subrCl</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p12s1w8">
                              <m.rf id="m-c70-Cairo-p12s1w8">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p12w8">
                                  <token>uzrakstīja</token>
                                  <no_space_after>1</no_space_after>
                                </w.rf>
                                <form>uzrakstīja</form>
                                <lemma>uzrakstīt</lemma>
                                <tag>vmnist330an</tag>
                              </m.rf>
                              <role>pred</role>
                              <ord>8</ord>
                              <children>
                                <node id="a-c70-Cairo-p12s1w6">
                                  <m.rf id="m-c70-Cairo-p12s1w6">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p12w6">
                                      <token>kurš</token>
                                    </w.rf>
                                    <form>kurš</form>
                                    <lemma>kurš</lemma>
                                    <tag>pr0msnn</tag>
                                  </m.rf>
                                  <role>subj</role>
                                  <ord>6</ord>
                                </node>
                                <node id="a-c70-Cairo-p12s1w7">
                                  <m.rf id="m-c70-Cairo-p12s1w7">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p12w7">
                                      <token>to</token>
                                    </w.rf>
                                    <form>to</form>
                                    <lemma>tas</lemma>
                                    <tag>pd3msan</tag>
                                  </m.rf>
                                  <role>obj</role>
                                  <ord>7</ord>
                                </node>
                              </children>
                            </node>
                            <node id="a-c70-Cairo-p12s1w5">
                              <m.rf id="m-c70-Cairo-p12s1w5">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p12w5">
                                  <token>,</token>
                                </w.rf>
                                <form>,</form>
                                <lemma>,</lemma>
                                <tag>zc</tag>
                              </m.rf>
                              <role>punct</role>
                              <ord>5</ord>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                    <xinfo>
                      <xtype>xParticle</xtype>
                      <tag>ncfsg0[neg]</tag>
                      <children>
                        <node id="a-c70-Cairo-p12s1w3">
                          <m.rf id="m-c70-Cairo-p12s1w3">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p12w3">
                              <token>ne</token>
                            </w.rf>
                            <form>ne</form>
                            <lemma>ne</lemma>
                            <tag>q</tag>
                          </m.rf>
                          <role>no</role>
                          <ord>3</ord>
                        </node>
                        <node id="a-c70-Cairo-p12s1w4">
                          <m.rf id="m-c70-Cairo-p12s1w4">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p12w4">
                              <token>jausmas</token>
                              <no_space_after>1</no_space_after>
                            </w.rf>
                            <form>jausmas</form>
                            <lemma>jausma</lemma>
                            <tag>ncfsg4</tag>
                          </m.rf>
                          <role>basElem</role>
                          <ord>4</ord>
                        </node>
                      </children>
                    </xinfo>
                  </children>
                </node>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p13s1">
      <s.rf>m#m-c70-Cairo-p13s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p13s1w5">
              <m.rf id="m-c70-Cairo-p13s1w5">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p13w5">
                  <token>?</token>
                </w.rf>
                <form>?</form>
                <lemma>?</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>5</ord>
            </node>
            <node id="a-c70-Cairo-p13s1w4">
              <m.rf id="m-c70-Cairo-p13s1w4">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p13w4">
                  <token>skaties</token>
                  <no_space_after>1</no_space_after>
                </w.rf>
                <form>skaties</form>
                <lemma>skatīties</lemma>
                <tag>vmyipt32san</tag>
              </m.rf>
              <role>pred</role>
              <ord>4</ord>
              <children>
                <node id="a-c70-Cairo-p13s1x1">
                  <role>adv</role>
                  <children>
                    <xinfo>
                      <xtype>xPrep</xtype>
                      <tag>pr000an[pre]</tag>
                      <children>
                        <node id="a-c70-Cairo-p13s1w2">
                          <m.rf id="m-c70-Cairo-p13s1w2">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p13w2">
                              <token>ko</token>
                            </w.rf>
                            <form>ko</form>
                            <lemma>kas</lemma>
                            <tag>pq000an</tag>
                          </m.rf>
                          <role>basElem</role>
                          <ord>2</ord>
                        </node>
                        <node id="a-c70-Cairo-p13s1w1">
                          <m.rf id="m-c70-Cairo-p13s1w1">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p13w1">
                              <token>Uz</token>
                            </w.rf>
                            <form>Uz</form>
                            <lemma>uz</lemma>
                            <tag>spsa</tag>
                          </m.rf>
                          <role>prep</role>
                          <ord>1</ord>
                        </node>
                      </children>
                    </xinfo>
                  </children>
                </node>
                <node id="a-c70-Cairo-p13s1w3">
                  <m.rf id="m-c70-Cairo-p13s1w3">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p13w3">
                      <token>tu</token>
                    </w.rf>
                    <form>tu</form>
                    <lemma>tu</lemma>
                    <tag>pp20snn</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>3</ord>
                </node>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p14s1">
      <s.rf>m#m-c70-Cairo-p14s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p14s1w9">
              <m.rf id="m-c70-Cairo-p14s1w9">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p14w9">
                  <token>?</token>
                </w.rf>
                <form>?</form>
                <lemma>?</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>9</ord>
            </node>
            <node id="a-c70-Cairo-p14s1w3">
              <m.rf id="m-c70-Cairo-p14s1w3">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p14w3">
                  <token>domā</token>
                  <no_space_after>1</no_space_after>
                </w.rf>
                <form>domā</form>
                <lemma>domāt</lemma>
                <tag>vmnipt22san</tag>
              </m.rf>
              <role>pred</role>
              <ord>3</ord>
              <children>
                <node id="a-c70-Cairo-p14s1w1">
                  <m.rf id="m-c70-Cairo-p14s1w1">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p14w1">
                      <token>Kā</token>
                    </w.rf>
                    <form>Kā</form>
                    <lemma>kā</lemma>
                    <tag>r0n</tag>
                  </m.rf>
                  <role>adv</role>
                  <ord>1</ord>
                </node>
                <node id="a-c70-Cairo-p14s1x1">
                  <role>objCl</role>
                  <children>
                    <pmcinfo>
                      <pmctype>subrCl</pmctype>
                      <children>
                        <node id="a-c70-Cairo-p14s1x2">
                          <role>pred</role>
                          <children>
                            <node id="a-c70-Cairo-p14s1w5">
                              <m.rf id="m-c70-Cairo-p14s1w5">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p14w5">
                                  <token>kad</token>
                                </w.rf>
                                <form>kad</form>
                                <lemma>kad</lemma>
                                <tag>r0n</tag>
                              </m.rf>
                              <role>adv</role>
                              <ord>5</ord>
                            </node>
                            <node id="a-c70-Cairo-p14s1w6">
                              <m.rf id="m-c70-Cairo-p14s1w6">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p14w6">
                                  <token>tu</token>
                                </w.rf>
                                <form>tu</form>
                                <lemma>tu</lemma>
                                <tag>pp20snn</tag>
                              </m.rf>
                              <role>subj</role>
                              <ord>6</ord>
                            </node>
                            <xinfo>
                              <xtype>xPred</xtype>
                              <tag>vm0if002san[modals00]</tag>
                              <children>
                                <node id="a-c70-Cairo-p14s1w7">
                                  <m.rf id="m-c70-Cairo-p14s1w7">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p14w7">
                                      <token>varēsi</token>
                                    </w.rf>
                                    <form>varēsi</form>
                                    <lemma>varēt</lemma>
                                    <tag>vonifi32san</tag>
                                  </m.rf>
                                  <role>mod</role>
                                  <ord>7</ord>
                                </node>
                                <node id="a-c70-Cairo-p14s1w8">
                                  <m.rf id="m-c70-Cairo-p14s1w8">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p14w8">
                                      <token>atnākt</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>atnākt</form>
                                    <lemma>atnākt</lemma>
                                    <tag>vmnn0i1000n</tag>
                                  </m.rf>
                                  <role>basElem</role>
                                  <ord>8</ord>
                                </node>
                              </children>
                            </xinfo>
                          </children>
                        </node>
                        <node id="a-c70-Cairo-p14s1w4">
                          <m.rf id="m-c70-Cairo-p14s1w4">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p14w4">
                              <token>,</token>
                            </w.rf>
                            <form>,</form>
                            <lemma>,</lemma>
                            <tag>zc</tag>
                          </m.rf>
                          <role>punct</role>
                          <ord>4</ord>
                        </node>
                      </children>
                    </pmcinfo>
                  </children>
                </node>
                <node id="a-c70-Cairo-p14s1w2">
                  <m.rf id="m-c70-Cairo-p14s1w2">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p14w2">
                      <token>tu</token>
                    </w.rf>
                    <form>tu</form>
                    <lemma>tu</lemma>
                    <tag>pp20snn</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>2</ord>
                </node>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p15s1">
      <s.rf>m#m-c70-Cairo-p15s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p15s1x1">
              <role>basElem</role>
              <children>
                <coordinfo>
                  <coordtype>crdClauses</coordtype>
                  <children>
                    <node id="a-c70-Cairo-p15s1x2">
                      <role>crdPart</role>
                      <children>
                        <pmcinfo>
                          <pmctype>mainCl</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p15s1w2">
                              <m.rf id="m-c70-Cairo-p15s1w2">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p15w2">
                                  <token>nopirka</token>
                                </w.rf>
                                <form>nopirka</form>
                                <lemma>nopirkt</lemma>
                                <tag>vmnist130an</tag>
                              </m.rf>
                              <role>pred</role>
                              <ord>2</ord>
                              <children>
                                <node id="a-c70-Cairo-p15s1w3">
                                  <m.rf id="m-c70-Cairo-p15s1w3">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p15w3">
                                      <token>mašīnu</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>mašīnu</form>
                                    <lemma>mašīna</lemma>
                                    <tag>ncfsa4</tag>
                                  </m.rf>
                                  <role>obj</role>
                                  <ord>3</ord>
                                </node>
                                <node id="a-c70-Cairo-p15s1w1">
                                  <m.rf id="m-c70-Cairo-p15s1w1">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p15w1">
                                      <token>Viņš</token>
                                    </w.rf>
                                    <form>Viņš</form>
                                    <lemma>viņš</lemma>
                                    <tag>pp3msnn</tag>
                                  </m.rf>
                                  <role>subj</role>
                                  <ord>1</ord>
                                </node>
                              </children>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p15s1x3">
                      <role>crdPart</role>
                      <children>
                        <pmcinfo>
                          <pmctype>mainCl</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p15s1x4">
                              <reduction>vmnist130an(nopirka)</reduction>
                              <role>pred</role>
                              <children>
                                <node id="a-c70-Cairo-p15s1w7">
                                  <m.rf id="m-c70-Cairo-p15s1w7">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p15w7">
                                      <token>brālis</token>
                                    </w.rf>
                                    <form>brālis</form>
                                    <lemma>brālis</lemma>
                                    <tag>ncmsn2</tag>
                                  </m.rf>
                                  <role>subj</role>
                                  <ord>7</ord>
                                  <children>
                                    <node id="a-c70-Cairo-p15s1w6">
                                      <m.rf id="m-c70-Cairo-p15s1w6">
                                        <src.rf>semi-automatic</src.rf>
                                        <w.rf id="w-c70-Cairo-p15w6">
                                          <token>viņa</token>
                                        </w.rf>
                                        <form>viņa</form>
                                        <lemma>viņš</lemma>
                                        <tag>pp3msgn</tag>
                                      </m.rf>
                                      <role>attr</role>
                                      <ord>6</ord>
                                    </node>
                                  </children>
                                </node>
                                <node id="a-c70-Cairo-p15s1w9">
                                  <m.rf id="m-c70-Cairo-p15s1w9">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p15w9">
                                      <token>velosipēdu</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>velosipēdu</form>
                                    <lemma>velosipēds</lemma>
                                    <tag>ncmsa1</tag>
                                  </m.rf>
                                  <role>obj</role>
                                  <ord>9</ord>
                                  <children>
                                    <node id="a-c70-Cairo-p15s1w8">
                                      <m.rf id="m-c70-Cairo-p15s1w8">
                                        <src.rf>semi-automatic</src.rf>
                                        <w.rf id="w-c70-Cairo-p15w8">
                                          <token>tikai</token>
                                        </w.rf>
                                        <form>tikai</form>
                                        <lemma>tikai</lemma>
                                        <tag>q</tag>
                                      </m.rf>
                                      <role>no</role>
                                      <ord>8</ord>
                                    </node>
                                  </children>
                                </node>
                              </children>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p15s1w4">
                      <m.rf id="m-c70-Cairo-p15s1w4">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p15w4">
                          <token>,</token>
                        </w.rf>
                        <form>,</form>
                        <lemma>,</lemma>
                        <tag>zc</tag>
                      </m.rf>
                      <role>punct</role>
                      <ord>4</ord>
                    </node>
                    <node id="a-c70-Cairo-p15s1w5">
                      <m.rf id="m-c70-Cairo-p15s1w5">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p15w5">
                          <token>bet</token>
                        </w.rf>
                        <form>bet</form>
                        <lemma>bet</lemma>
                        <tag>cc</tag>
                      </m.rf>
                      <role>conj</role>
                      <ord>5</ord>
                    </node>
                  </children>
                </coordinfo>
              </children>
            </node>
            <node id="a-c70-Cairo-p15s1w10">
              <m.rf id="m-c70-Cairo-p15s1w10">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p15w10">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>10</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p16s1">
      <s.rf>m#m-c70-Cairo-p16s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p16s1x1">
              <role>pred</role>
              <children>
                <coordinfo>
                  <coordtype>crdParts</coordtype>
                  <children>
                    <node id="a-c70-Cairo-p16s1w5">
                      <m.rf id="m-c70-Cairo-p16s1w5">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p16w5">
                          <token>un</token>
                        </w.rf>
                        <form>un</form>
                        <lemma>un</lemma>
                        <tag>cc</tag>
                      </m.rf>
                      <role>conj</role>
                      <ord>5</ord>
                    </node>
                    <node id="a-c70-Cairo-p16s1w7">
                      <m.rf id="m-c70-Cairo-p16s1w7">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p16w7">
                          <token>atstāja</token>
                        </w.rf>
                        <form>atstāja</form>
                        <lemma>atstāt</lemma>
                        <tag>vmnist130an</tag>
                      </m.rf>
                      <role>crdPart</role>
                      <ord>7</ord>
                      <children>
                        <node id="a-c70-Cairo-p16s1w6">
                          <m.rf id="m-c70-Cairo-p16s1w6">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p16w6">
                              <token>tad</token>
                            </w.rf>
                            <form>tad</form>
                            <lemma>tad</lemma>
                            <tag>r0n</tag>
                          </m.rf>
                          <role>adv</role>
                          <ord>6</ord>
                        </node>
                        <node id="a-c70-Cairo-p16s1w8">
                          <m.rf id="m-c70-Cairo-p16s1w8">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p16w8">
                              <token>istabu</token>
                              <no_space_after>1</no_space_after>
                            </w.rf>
                            <form>istabu</form>
                            <lemma>istaba</lemma>
                            <tag>ncfsa4</tag>
                          </m.rf>
                          <role>obj</role>
                          <ord>8</ord>
                        </node>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p16s1w4">
                      <m.rf id="m-c70-Cairo-p16s1w4">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p16w4">
                          <token>apskāvās</token>
                        </w.rf>
                        <form>apskāvās</form>
                        <lemma>apskauties</lemma>
                        <tag>vmyisi130an</tag>
                      </m.rf>
                      <role>crdPart</role>
                      <ord>4</ord>
                    </node>
                  </children>
                </coordinfo>
                <node id="a-c70-Cairo-p16s1x2">
                  <role>subj</role>
                  <children>
                    <coordinfo>
                      <coordtype>crdParts</coordtype>
                      <children>
                        <node id="a-c70-Cairo-p16s1w1">
                          <m.rf id="m-c70-Cairo-p16s1w1">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p16w1">
                              <token>Pēteris</token>
                            </w.rf>
                            <form>Pēteris</form>
                            <lemma>Pēteris</lemma>
                            <tag>npmsn2</tag>
                          </m.rf>
                          <role>crdPart</role>
                          <ord>1</ord>
                        </node>
                        <node id="a-c70-Cairo-p16s1w3">
                          <m.rf id="m-c70-Cairo-p16s1w3">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p16w3">
                              <token>Marija</token>
                            </w.rf>
                            <form>Marija</form>
                            <lemma>Marija</lemma>
                            <tag>npfsn4</tag>
                          </m.rf>
                          <role>crdPart</role>
                          <ord>3</ord>
                        </node>
                        <node id="a-c70-Cairo-p16s1w2">
                          <m.rf id="m-c70-Cairo-p16s1w2">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p16w2">
                              <token>un</token>
                            </w.rf>
                            <form>un</form>
                            <lemma>un</lemma>
                            <tag>cc</tag>
                          </m.rf>
                          <role>conj</role>
                          <ord>2</ord>
                        </node>
                      </children>
                    </coordinfo>
                  </children>
                </node>
              </children>
            </node>
            <node id="a-c70-Cairo-p16s1w9">
              <m.rf id="m-c70-Cairo-p16s1w9">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p16w9">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>9</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p17s1">
      <s.rf>m#m-c70-Cairo-p17s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p17s1x1">
              <role>basElem</role>
              <children>
                <coordinfo>
                  <coordtype>crdClauses</coordtype>
                  <children>
                    <node id="a-c70-Cairo-p17s1x2">
                      <role>crdPart</role>
                      <children>
                        <node id="a-c70-Cairo-p17s1w1">
                          <m.rf id="m-c70-Cairo-p17s1w1">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p17w1">
                              <token>Viņai</token>
                            </w.rf>
                            <form>Viņai</form>
                            <lemma>viņa</lemma>
                            <tag>pp3fsdn</tag>
                          </m.rf>
                          <role>det</role>
                          <ord>1</ord>
                        </node>
                        <pmcinfo>
                          <pmctype>mainCl</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p17s1x4">
                              <role>pred</role>
                              <children>
                                <xinfo>
                                  <xtype>xPred</xtype>
                                  <tag>vm0is0030an[modals00]</tag>
                                  <children>
                                    <node id="a-c70-Cairo-p17s1w2">
                                      <m.rf id="m-c70-Cairo-p17s1w2">
                                        <src.rf>semi-automatic</src.rf>
                                        <w.rf id="w-c70-Cairo-p17w2">
                                          <token>vajadzēja</token>
                                        </w.rf>
                                        <form>vajadzēja</form>
                                        <lemma>vajadzēt</lemma>
                                        <tag>vonisi330an</tag>
                                      </m.rf>
                                      <role>mod</role>
                                      <ord>2</ord>
                                    </node>
                                    <node id="a-c70-Cairo-p17s1w3">
                                      <m.rf id="m-c70-Cairo-p17s1w3">
                                        <src.rf>semi-automatic</src.rf>
                                        <w.rf id="w-c70-Cairo-p17w3">
                                          <token>apgriezt</token>
                                        </w.rf>
                                        <form>apgriezt</form>
                                        <lemma>apgriezt</lemma>
                                        <tag>vmnn0t1000n</tag>
                                      </m.rf>
                                      <role>basElem</role>
                                      <ord>3</ord>
                                      <children>
                                        <node id="a-c70-Cairo-p17s1w4">
                                          <m.rf id="m-c70-Cairo-p17s1w4">
                                            <src.rf>semi-automatic</src.rf>
                                            <w.rf id="w-c70-Cairo-p17w4">
                                              <token>matus</token>
                                              <no_space_after>1</no_space_after>
                                            </w.rf>
                                            <form>matus</form>
                                            <lemma>mats</lemma>
                                            <tag>ncmpa1</tag>
                                          </m.rf>
                                          <role>obj</role>
                                          <ord>4</ord>
                                        </node>
                                      </children>
                                    </node>
                                  </children>
                                </xinfo>
                              </children>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p17s1x3">
                      <role>crdPart</role>
                      <children>
                        <pmcinfo>
                          <pmctype>mainCl</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p17s1w14">
                              <m.rf id="m-c70-Cairo-p17s1w14">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p17w14">
                                  <token>neizdarīja</token>
                                  <no_space_after>1</no_space_after>
                                </w.rf>
                                <form>neizdarīja</form>
                                <lemma>izdarīt</lemma>
                                <tag>vmnist330ay</tag>
                              </m.rf>
                              <role>pred</role>
                              <ord>14</ord>
                              <children>
                                <node id="a-c70-Cairo-p17s1x5">
                                  <role>adv</role>
                                  <children>
                                    <xinfo>
                                      <xtype>xPrep</xtype>
                                      <tag>ncmsg0[post]</tag>
                                      <children>
                                        <node id="a-c70-Cairo-p17s1w9">
                                          <m.rf id="m-c70-Cairo-p17s1w9">
                                            <src.rf>semi-automatic</src.rf>
                                            <w.rf id="w-c70-Cairo-p17w9">
                                              <token>iemesla</token>
                                            </w.rf>
                                            <form>iemesla</form>
                                            <lemma>iemesls</lemma>
                                            <tag>ncmsg1</tag>
                                          </m.rf>
                                          <role>basElem</role>
                                          <ord>9</ord>
                                          <children>
                                            <node id="a-c70-Cairo-p17s1x6">
                                              <role>attr</role>
                                              <children>
                                                <xinfo>
                                                  <xtype>xParticle</xtype>
                                                  <tag>pi0msgn[aff]</tag>
                                                  <children>
                                                    <node id="a-c70-Cairo-p17s1w7">
                                                      <m.rf id="m-c70-Cairo-p17s1w7">
                                                        <src.rf>semi-automatic</src.rf>
                                                        <w.rf id="w-c70-Cairo-p17w7">
                                                          <token>kaut</token>
                                                        </w.rf>
                                                        <form>kaut</form>
                                                        <lemma>kaut</lemma>
                                                        <tag>q</tag>
                                                      </m.rf>
                                                      <role>no</role>
                                                      <ord>7</ord>
                                                    </node>
                                                    <node id="a-c70-Cairo-p17s1w8">
                                                      <m.rf id="m-c70-Cairo-p17s1w8">
                                                        <src.rf>semi-automatic</src.rf>
                                                        <w.rf id="w-c70-Cairo-p17w8">
                                                          <token>kāda</token>
                                                        </w.rf>
                                                        <form>kāda</form>
                                                        <lemma>kāds</lemma>
                                                        <tag>pi0msgn</tag>
                                                      </m.rf>
                                                      <role>basElem</role>
                                                      <ord>8</ord>
                                                    </node>
                                                  </children>
                                                </xinfo>
                                              </children>
                                            </node>
                                          </children>
                                        </node>
                                        <node id="a-c70-Cairo-p17s1w10">
                                          <m.rf id="m-c70-Cairo-p17s1w10">
                                            <src.rf>semi-automatic</src.rf>
                                            <w.rf id="w-c70-Cairo-p17w10">
                                              <token>dēļ</token>
                                            </w.rf>
                                            <form>dēļ</form>
                                            <lemma>dēļ</lemma>
                                            <tag>stsg</tag>
                                          </m.rf>
                                          <role>prep</role>
                                          <ord>10</ord>
                                        </node>
                                      </children>
                                    </xinfo>
                                  </children>
                                </node>
                                <node id="a-c70-Cairo-p17s1w12">
                                  <m.rf id="m-c70-Cairo-p17s1w12">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p17w12">
                                      <token>todien</token>
                                    </w.rf>
                                    <form>todien</form>
                                    <lemma>todien</lemma>
                                    <tag>r0n</tag>
                                  </m.rf>
                                  <role>adv</role>
                                  <ord>12</ord>
                                </node>
                                <node id="a-c70-Cairo-p17s1w13">
                                  <m.rf id="m-c70-Cairo-p17s1w13">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p17w13">
                                      <token>to</token>
                                    </w.rf>
                                    <form>to</form>
                                    <lemma>tas</lemma>
                                    <tag>pd3msan</tag>
                                  </m.rf>
                                  <role>obj</role>
                                  <ord>13</ord>
                                </node>
                                <node id="a-c70-Cairo-p17s1w11">
                                  <m.rf id="m-c70-Cairo-p17s1w11">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p17w11">
                                      <token>viņa</token>
                                    </w.rf>
                                    <form>viņa</form>
                                    <lemma>viņa</lemma>
                                    <tag>pp3fsnn</tag>
                                  </m.rf>
                                  <role>subj</role>
                                  <ord>11</ord>
                                </node>
                              </children>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p17s1w5">
                      <m.rf id="m-c70-Cairo-p17s1w5">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p17w5">
                          <token>,</token>
                        </w.rf>
                        <form>,</form>
                        <lemma>,</lemma>
                        <tag>zc</tag>
                      </m.rf>
                      <role>punct</role>
                      <ord>5</ord>
                    </node>
                    <node id="a-c70-Cairo-p17s1w6">
                      <m.rf id="m-c70-Cairo-p17s1w6">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p17w6">
                          <token>bet</token>
                        </w.rf>
                        <form>bet</form>
                        <lemma>bet</lemma>
                        <tag>cc</tag>
                      </m.rf>
                      <role>conj</role>
                      <ord>6</ord>
                    </node>
                  </children>
                </coordinfo>
              </children>
            </node>
            <node id="a-c70-Cairo-p17s1w15">
              <m.rf id="m-c70-Cairo-p17s1w15">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p17w15">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>15</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p18s1">
      <s.rf>m#m-c70-Cairo-p18s1</s.rf>
      <children>
        <node id="a-c70-Cairo-p18s1x1">
          <role>motivCl</role>
          <children>
            <pmcinfo>
              <pmctype>subrCl</pmctype>
              <children>
                <node id="a-c70-Cairo-p18s1w6">
                  <m.rf id="m-c70-Cairo-p18s1w6">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p18w6">
                      <token>,</token>
                    </w.rf>
                    <form>,</form>
                    <lemma>,</lemma>
                    <tag>zc</tag>
                  </m.rf>
                  <role>punct</role>
                  <ord>6</ord>
                </node>
                <node id="a-c70-Cairo-p18s1w7">
                  <m.rf id="m-c70-Cairo-p18s1w7">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p18w7">
                      <token>jo</token>
                    </w.rf>
                    <form>jo</form>
                    <lemma>jo</lemma>
                    <tag>cs</tag>
                  </m.rf>
                  <role>conj</role>
                  <ord>7</ord>
                </node>
                <node id="a-c70-Cairo-p18s1w9">
                  <m.rf id="m-c70-Cairo-p18s1w9">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p18w9">
                      <token>skrēja</token>
                    </w.rf>
                    <form>skrēja</form>
                    <lemma>skriet</lemma>
                    <tag>vmnist130an</tag>
                  </m.rf>
                  <role>pred</role>
                  <ord>9</ord>
                  <children>
                    <node id="a-c70-Cairo-p18s1w8">
                      <m.rf id="m-c70-Cairo-p18s1w8">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p18w8">
                          <token>viņš</token>
                        </w.rf>
                        <form>viņš</form>
                        <lemma>viņš</lemma>
                        <tag>pp3msnn</tag>
                      </m.rf>
                      <role>subj</role>
                      <ord>8</ord>
                    </node>
                    <node id="a-c70-Cairo-p18s1w11">
                      <m.rf id="m-c70-Cairo-p18s1w11">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p18w11">
                          <token>ātri</token>
                          <no_space_after>1</no_space_after>
                        </w.rf>
                        <form>ātri</form>
                        <lemma>ātri</lemma>
                        <tag>rpn</tag>
                      </m.rf>
                      <role>adv</role>
                      <ord>11</ord>
                      <children>
                        <node id="a-c70-Cairo-p18s1w10">
                          <m.rf id="m-c70-Cairo-p18s1w10">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p18w10">
                              <token>pārāk</token>
                            </w.rf>
                            <form>pārāk</form>
                            <lemma>pārāk</lemma>
                            <tag>r0n</tag>
                          </m.rf>
                          <role>adv</role>
                          <ord>10</ord>
                        </node>
                      </children>
                    </node>
                  </children>
                </node>
              </children>
            </pmcinfo>
          </children>
        </node>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p18s1x2">
              <role>pred</role>
              <children>
                <node id="a-c70-Cairo-p18s1w1">
                  <m.rf id="m-c70-Cairo-p18s1w1">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p18w1">
                      <token>Es</token>
                    </w.rf>
                    <form>Es</form>
                    <lemma>es</lemma>
                    <tag>pp10snn</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>1</ord>
                </node>
                <xinfo>
                  <xtype>xPred</xtype>
                  <tag>vm0is0030ay[modals00]</tag>
                  <children>
                    <node id="a-c70-Cairo-p18s1w2">
                      <m.rf id="m-c70-Cairo-p18s1w2">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p18w2">
                          <token>nevarēju</token>
                        </w.rf>
                        <form>nevarēju</form>
                        <lemma>varēt</lemma>
                        <tag>vonisi31say</tag>
                      </m.rf>
                      <role>mod</role>
                      <ord>2</ord>
                    </node>
                    <node id="a-c70-Cairo-p18s1w3">
                      <m.rf id="m-c70-Cairo-p18s1w3">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p18w3">
                          <token>tikt</token>
                        </w.rf>
                        <form>tikt</form>
                        <lemma>tikt</lemma>
                        <tag>vmnn0i1000n</tag>
                      </m.rf>
                      <role>basElem</role>
                      <ord>3</ord>
                      <children>
                        <node id="a-c70-Cairo-p18s1x3">
                          <role>adv</role>
                          <children>
                            <xinfo>
                              <xtype>xPrep</xtype>
                              <tag>pp0msdn[rel]</tag>
                              <children>
                                <node id="a-c70-Cairo-p18s1w4">
                                  <m.rf id="m-c70-Cairo-p18s1w4">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p18w4">
                                      <token>viņam</token>
                                    </w.rf>
                                    <form>viņam</form>
                                    <lemma>viņš</lemma>
                                    <tag>pp3msdn</tag>
                                  </m.rf>
                                  <role>basElem</role>
                                  <ord>4</ord>
                                </node>
                                <node id="a-c70-Cairo-p18s1w5">
                                  <m.rf id="m-c70-Cairo-p18s1w5">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p18w5">
                                      <token>līdzi</token>
                                      <no_space_after>1</no_space_after>
                                    </w.rf>
                                    <form>līdzi</form>
                                    <lemma>līdzi</lemma>
                                    <tag>r0y</tag>
                                  </m.rf>
                                  <role>prep</role>
                                  <ord>5</ord>
                                </node>
                              </children>
                            </xinfo>
                          </children>
                        </node>
                      </children>
                    </node>
                  </children>
                </xinfo>
              </children>
            </node>
            <node id="a-c70-Cairo-p18s1w12">
              <m.rf id="m-c70-Cairo-p18s1w12">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p18w12">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>12</ord>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p19s1">
      <s.rf>m#m-c70-Cairo-p19s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p19s1w10">
              <m.rf id="m-c70-Cairo-p19s1w10">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p19w10">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>10</ord>
            </node>
            <node id="a-c70-Cairo-p19s1x1">
              <role>pred</role>
              <children>
                <coordinfo>
                  <coordtype>crdParts</coordtype>
                  <children>
                    <node id="a-c70-Cairo-p19s1x2">
                      <role>crdPart</role>
                      <children>
                        <xinfo>
                          <xtype>xPred</xtype>
                          <tag>vm0ip003san[substsmg]</tag>
                          <children>
                            <node id="a-c70-Cairo-p19s1w3">
                              <m.rf id="m-c70-Cairo-p19s1w3">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p19w3">
                                  <token>ir</token>
                                </w.rf>
                                <form>ir</form>
                                <lemma>būt</lemma>
                                <tag>vcnipii30an</tag>
                              </m.rf>
                              <role>auxVerb</role>
                              <ord>3</ord>
                            </node>
                            <node id="a-c70-Cairo-p19s1x4">
                              <role>basElem</role>
                              <children>
                                <xinfo>
                                  <xtype>xPrep</xtype>
                                  <tag>npmsg0[pre]</tag>
                                  <children>
                                    <node id="a-c70-Cairo-p19s1w4">
                                      <m.rf id="m-c70-Cairo-p19s1w4">
                                        <src.rf>semi-automatic</src.rf>
                                        <w.rf id="w-c70-Cairo-p19w4">
                                          <token>no</token>
                                        </w.rf>
                                        <form>no</form>
                                        <lemma>no</lemma>
                                        <tag>spsg</tag>
                                      </m.rf>
                                      <role>prep</role>
                                      <ord>4</ord>
                                    </node>
                                    <node id="a-c70-Cairo-p19s1w5">
                                      <m.rf id="m-c70-Cairo-p19s1w5">
                                        <src.rf>semi-automatic</src.rf>
                                        <w.rf id="w-c70-Cairo-p19w5">
                                          <token>Pētera</token>
                                        </w.rf>
                                        <form>Pētera</form>
                                        <lemma>Pēteris</lemma>
                                        <tag>npmsg2</tag>
                                      </m.rf>
                                      <role>basElem</role>
                                      <ord>5</ord>
                                    </node>
                                  </children>
                                </xinfo>
                              </children>
                            </node>
                          </children>
                        </xinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p19s1x3">
                      <role>crdPart</role>
                      <children>
                        <node id="a-c70-Cairo-p19s1w9">
                          <m.rf id="m-c70-Cairo-p19s1w9">
                            <src.rf>semi-automatic</src.rf>
                            <w.rf id="w-c70-Cairo-p19w9">
                              <token>vakar</token>
                              <no_space_after>1</no_space_after>
                            </w.rf>
                            <form>vakar</form>
                            <lemma>vakar</lemma>
                            <tag>r0n</tag>
                          </m.rf>
                          <role>adv</role>
                          <ord>9</ord>
                        </node>
                        <xinfo>
                          <xtype>xPred</xtype>
                          <tag>vm0is003spn[passsfn]</tag>
                          <children>
                            <node id="a-c70-Cairo-p19s1w7">
                              <m.rf id="m-c70-Cairo-p19s1w7">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p19w7">
                                  <token>tika</token>
                                </w.rf>
                                <form>tika</form>
                                <lemma>tikt</lemma>
                                <tag>vtnisi130an</tag>
                              </m.rf>
                              <role>auxVerb</role>
                              <ord>7</ord>
                            </node>
                            <node id="a-c70-Cairo-p19s1w8">
                              <m.rf id="m-c70-Cairo-p19s1w8">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p19w8">
                                  <token>piegādāta</token>
                                </w.rf>
                                <form>piegādāta</form>
                                <lemma>piegādāt</lemma>
                                <tag>vmnpdfsnpsnpn</tag>
                              </m.rf>
                              <role>basElem</role>
                              <ord>8</ord>
                            </node>
                          </children>
                        </xinfo>
                      </children>
                    </node>
                    <node id="a-c70-Cairo-p19s1w6">
                      <m.rf id="m-c70-Cairo-p19s1w6">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p19w6">
                          <token>un</token>
                        </w.rf>
                        <form>un</form>
                        <lemma>un</lemma>
                        <tag>cc</tag>
                      </m.rf>
                      <role>conj</role>
                      <ord>6</ord>
                    </node>
                  </children>
                </coordinfo>
                <node id="a-c70-Cairo-p19s1w2">
                  <m.rf id="m-c70-Cairo-p19s1w2">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p19w2">
                      <token>vēstule</token>
                    </w.rf>
                    <form>vēstule</form>
                    <lemma>vēstule</lemma>
                    <tag>ncfsn5</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>2</ord>
                  <children>
                    <node id="a-c70-Cairo-p19s1w1">
                      <m.rf id="m-c70-Cairo-p19s1w1">
                        <src.rf>semi-automatic</src.rf>
                        <w.rf id="w-c70-Cairo-p19w1">
                          <token>Šī</token>
                        </w.rf>
                        <form>Šī</form>
                        <lemma>šī</lemma>
                        <tag>pd3fsnn</tag>
                      </m.rf>
                      <role>attr</role>
                      <ord>1</ord>
                    </node>
                  </children>
                </node>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
    <LM id="a-c70-Cairo-p20s1">
      <s.rf>m#m-c70-Cairo-p20s1</s.rf>
      <children>
        <pmcinfo>
          <pmctype>sent</pmctype>
          <children>
            <node id="a-c70-Cairo-p20s1w7">
              <m.rf id="m-c70-Cairo-p20s1w7">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p20w7">
                  <token>.</token>
                </w.rf>
                <form>.</form>
                <lemma>.</lemma>
                <tag>zs</tag>
              </m.rf>
              <role>punct</role>
              <ord>7</ord>
            </node>
            <node id="a-c70-Cairo-p20s1w2">
              <m.rf id="m-c70-Cairo-p20s1w2">
                <src.rf>semi-automatic</src.rf>
                <w.rf id="w-c70-Cairo-p20w2">
                  <token>uzauga</token>
                </w.rf>
                <form>uzauga</form>
                <lemma>uzaugt</lemma>
                <tag>vmnisi130an</tag>
              </m.rf>
              <role>pred</role>
              <ord>2</ord>
              <children>
                <node id="a-c70-Cairo-p20s1w1">
                  <m.rf id="m-c70-Cairo-p20s1w1">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p20w1">
                      <token>Viņa</token>
                    </w.rf>
                    <form>Viņa</form>
                    <lemma>viņa</lemma>
                    <tag>pp3fsnn</tag>
                  </m.rf>
                  <role>subj</role>
                  <ord>1</ord>
                </node>
                <node id="a-c70-Cairo-p20s1w3">
                  <m.rf id="m-c70-Cairo-p20s1w3">
                    <src.rf>semi-automatic</src.rf>
                    <w.rf id="w-c70-Cairo-p20w3">
                      <token>Parīzē</token>
                      <no_space_after>1</no_space_after>
                    </w.rf>
                    <form>Parīzē</form>
                    <lemma>Parīze</lemma>
                    <tag>npfsl5</tag>
                  </m.rf>
                  <role>adv</role>
                  <ord>3</ord>
                  <children>
                    <node id="a-c70-Cairo-p20s1x1">
                      <role>spc</role>
                      <children>
                        <pmcinfo>
                          <pmctype>spcPmc</pmctype>
                          <children>
                            <node id="a-c70-Cairo-p20s1w4">
                              <m.rf id="m-c70-Cairo-p20s1w4">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p20w4">
                                  <token>,</token>
                                </w.rf>
                                <form>,</form>
                                <lemma>,</lemma>
                                <tag>zc</tag>
                              </m.rf>
                              <role>punct</role>
                              <ord>4</ord>
                            </node>
                            <node id="a-c70-Cairo-p20s1w6">
                              <m.rf id="m-c70-Cairo-p20s1w6">
                                <src.rf>semi-automatic</src.rf>
                                <w.rf id="w-c70-Cairo-p20w6">
                                  <token>galvaspilsētā</token>
                                  <no_space_after>1</no_space_after>
                                </w.rf>
                                <form>galvaspilsētā</form>
                                <lemma>galvaspilsēta</lemma>
                                <tag>ncfsl4</tag>
                              </m.rf>
                              <role>basElem</role>
                              <ord>6</ord>
                              <children>
                                <node id="a-c70-Cairo-p20s1w5">
                                  <m.rf id="m-c70-Cairo-p20s1w5">
                                    <src.rf>semi-automatic</src.rf>
                                    <w.rf id="w-c70-Cairo-p20w5">
                                      <token>Francijas</token>
                                    </w.rf>
                                    <form>Francijas</form>
                                    <lemma>Francija</lemma>
                                    <tag>npfsg4</tag>
                                  </m.rf>
                                  <role>attr</role>
                                  <ord>5</ord>
                                </node>
                              </children>
                            </node>
                          </children>
                        </pmcinfo>
                      </children>
                    </node>
                  </children>
                </node>
              </children>
            </node>
          </children>
        </pmcinfo>
      </children>
    </LM>
  </trees>
</lvadata>
