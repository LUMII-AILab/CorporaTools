...
sub copy_decl {
  ...
  my $copy;
  if (ref $t->{-schema}) {
    $copy = Fslib::CloneValue($t,[$t->{-parent},$t->{-schema}], [$self,$self->{-schema}]);
  } else {
    $copy = Fslib::CloneValue($t,[$t->{-parent}], [$self]);
  }
  if (exists $self->{'-##'}) {
    $copy->{'-#'}=$self->{'-##'}++;
  }
...
sub serialize_get_children {
  ...
  my @children = map {
    my $name = $_;
    my $val = $self->{$_};
    (ref($val) eq 'HASH')  ? ( map { [$name,$_] } grep { UNIVERSAL::isa($_,'PMLSchema::XMLNode') } values(%{$val})) :
    (ref($val) eq 'ARRAY') ? ( map { [$name,$_] } grep { UNIVERSAL::isa($_,'PMLSchema::XMLNode') } @{$val}) :
     (UNIVERSAL::isa($val,'PMLSchema::XMLNode') or !ref($val)) ? [$name,$val] : ()
...
sub serialize_children {
  ...
  for my $child (@$children) {
    my ($key,$value) = @$child;
    if (UNIVERSAL::isa($value,'PMLSchema::XMLNode')) {
...
sub write {
  ...
  if (!defined($opts->{fh}) and
    ...
    unless ($opts->{no_backups}) {
      eval { IOBackend::rename_uri($filename,$filename."~"); $have_backup=1; } || carp($@);
    }
    $fh = IOBackend::open_backend($filename,'w')
  ...
  eval {
    my $writer = XML::Writer->new(
      ...
      NAMESPACES => 1,
      PREFIX_MAP => {
	(PMLSchema->PML_SCHEMA_NS) => '',
      });
    $self->serialize({
      writer => $writer,
      DefaultNs => PMLSchema->PML_SCHEMA_NS,
    });
    $writer->end();
  };
  if ($@) {
    my $err=$@;
    $have_backup && eval { IOBackend::rename_uri($filename."~",$filename) };
    $err.=$@ if $@;
    carp("Error while saving schema: $err\n");
  }
  IOBackend::close_backend($fh) if $fh;
}
...
PMLSchema::XMLNode - base class for PMLSchema components
...
A common base class for components of a PMLSchema object.
...
This method serializes an object based on XMLNode the class to XML. See PMLSchema->write for details.
