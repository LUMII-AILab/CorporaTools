...
This class inherits from L<PMLSchema::XMLNode>.
...
#	  warn("Member $decl->{-parent}{-path}/$decl->{-name} with role=\"#KNIT\" must have a content type declaration: assuming <cdata format=\"PMLREF\">!\n");
	  PMLSchema::__fix_knit_type($schema,$mdecl);
	}
      }
    }
...
#      warn("List $sub->{-name} with role=\"#KNIT\" must have a content type declaration: assuming <cdata format=\"PMLREF\">!\n");
      PMLSchema::__fix_knit_type($schema,$sub,$path.'/LM');
    }
  } elsif ($sub = $decl->{alt}) {
    $decl_type = 'alt';
...
This method serializes a declaration to XML. See PMLSchema->write for
details and PMLSchema::XMLNode->write for implementation.
...
L<PMLSchema>, L<PMLSchema::XMLNode>
