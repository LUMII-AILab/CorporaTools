...
sub new {
  ...
  if ($opts->{validate}) {
    my $rng = $opts->{relaxng_schema} || Fslib::FindInResources('pml_schema_inline.rng');
    if (defined $rng) {
      push @common, (RelaxNG => $rng);
    } else {
      warn __PACKAGE__.": Validation requested, but 'pml_schema_inline.rng' was not found in the ResourcePath: ".Fslib::ResourcePath()."\n";
    }
  }
  my ($reader,$fh);
  ...
  } elsif (UNIVERSAL::isa($opts->{reader},'XML::LibXML::Reader')) {
    $reader = $opts->{reader};
    $URL ||= $reader->document->URI;
  } else {
    my $file = $opts->{URL};
    print STDERR "parsing schema $file\n" if $Fslib::Debug;
    $fh = eval { IOBackend::open_uri($file) };
...
PMLSchema::Reader - an auxiliary class for parsing PML schemas

=head1 DESCRIPTION

This class is used in the PMLSchema->new constructor to acutally parse
...
XML::Simple. PMLSchema::Reader uses XML::LibXML::Reader for XML
parsing.


=head1 SEE ALSO

L<PMLSchema>, L<XML::LibXML::Reader>, L<XML::Simple>
