...
use PMLSchema::XMLNode;
use PMLSchema::Decl;
use PMLSchema::Root;
use PMLSchema::Template;
use PMLSchema::Derive;
use PMLSchema::Copy;
use PMLSchema::Import;
use PMLSchema::Type;
use PMLSchema::Struct;
use PMLSchema::Container;
use PMLSchema::Seq;
use PMLSchema::List;
use PMLSchema::Alt;
use PMLSchema::Choice;
use PMLSchema::CDATA;
use PMLSchema::Constant;
use PMLSchema::Member;
use PMLSchema::Element;
use PMLSchema::Attribute;
use PMLSchema::Reader;
use XML::Writer;

use base qw(PMLSchema::Template);
...
This class inherits from L<PMLSchema::Template>.
...
=item PMLSchema->new ({ option => value, ... })

Parses a XML representation of a PMLSchema
...
and returns the corresponding C<PMLSchema> object.
...
locate referred schemas also in L<Fslib> resource paths.
...
file 'pml_schema_inline.rng' searched for in L<Fslib> resource paths
is assumed.
...
BEGIN{
  ...
  Bless => {
    member =>  'PMLSchema::Member',
    attribute =>  'PMLSchema::Attribute',
    element =>  'PMLSchema::Element',
    type =>  'PMLSchema::Type',
    root =>  'PMLSchema::Root',
    structure =>  'PMLSchema::Struct',
    container =>  'PMLSchema::Container',
    sequence =>  'PMLSchema::Seq',
    list =>  'PMLSchema::List',
    alt =>  'PMLSchema::Alt',
    cdata =>  'PMLSchema::CDATA',
    constant =>  'PMLSchema::Constant',
    choice =>  'PMLSchema::Choice',
    template => 'PMLSchema::Template',
    copy => 'PMLSchema::Copy',
    import => 'PMLSchema::Import',
    derive => 'PMLSchema::Derive',
    '*' => 'PMLSchema::XMLNode',
  },
...
sub new {
  ...
  if (defined $base and length $base) {
    $file = Fslib::ResolvePath($base,$file,$opts->{use_resources});
  ...
  my $pml_reader = PMLSchema::Reader->new($parse_opts);
  ...
  if ($@) {
    die "PMLSchema::Reader error while parsing: $file near line ".$reader->lineNumber."\n$@\n";
    return;
  }
  ...
  $schema->check_revision($revision_opts);
  $schema->{-VERSION}=$PMLSchema::VERSION;
  return $schema;
}
} # BEGIN
...
An obsolete alias for PMLSchema->new({%$opts, filename=>$filename}).
...
This method serializes the PMLSchema object to XML. See PMLSchema::XMLNode->write for implementation.
...
Return the  constant PML_SCHEMA_DECL (for compatibility with the PMLSchema::Decl interface).

=item $decl->get_decl_type_str ()

Return the string 'schema' (for compatibility with the PMLSchema::Decl interface).
...
sub _import_type {
  ...
  foreach my $n (keys %referred) {
    unless (exists $self->{type}{$n}) {
      ...
      if (defined $parent) {
	$self->{type}{$n}=Fslib::CloneValue($referred{$n},[$parent], [$self]);
      } else {
...
Return a list of declarations (objects derived from PMLSchema::Decl)
...
=item L<PMLSchema::Decl>

=item L<PMLSchema::Root>

=item L<PMLSchema::Type>

=item L<PMLSchema::Struct>

=item L<PMLSchema::Container>

=item L<PMLSchema::Seq>

=item L<PMLSchema::List>

=item L<PMLSchema::Alt>

=item L<PMLSchema::Choice>

=item L<PMLSchema::CDATA>

=item L<PMLSchema::Constant>

=item L<PMLSchema::Member>

=item L<PMLSchema::Element>

=item L<PMLSchema::Attribute>
