...
corpora. Native data format of the Latvian corpora is an adjusted PML.
...
* `PmlCorporaTools` - _perl_ scripts and xslt-s for varios coprora data
  processing;
* `LVTB2UD` - transformator from native annotation format to _Universal
  Dependencies_;
* `TrEd extension` - development snapshot for `lv-treebank` module for TrEd
  (this enables _TrEd_ to operate with PML files from _Latvian Treebank_);
* `SketchEngine specs` - specification files that are used together with `.vert`
  files generated by scripts in `PmlCorporaTools`;
* `TigerXML Schemas` (_obselote_) - schemas for displaying _Latvian Treebank_ in
  _Tiger Search_ (data transformation can be done, applying
  `PmlCorporaTools/LvCorporaTools/TreeTransf/lvpml2tiger.xsl` to "knitted" PML
  files, "knitting" can by done by `PmlCorporaTools/LvCorporaTools/UIs/TreeTransformatorUI.pm`).
...
Folders `ParserTools` and `ParserTools` deleted on 2026-03-02 as very outdated.
...
For LVTB2UD
* Java
* Morphological library https://github.com/LUMII-AILab/morphology
...
* `./preparePOSTagData.sh` is used for preparing morfological data for
  [LVTagger](https://github.com/LUMII-AILab/LVTagger)
